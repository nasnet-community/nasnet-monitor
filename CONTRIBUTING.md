...
`main/frontend/`, while the backend is a self-contained Go module at `main/backend/`. Run npm
commands from `main/` and run `go` / `make` from `main/backend/`.
...
environment (OS, Go and Node versions, and the dish/router firmware if relevant).
